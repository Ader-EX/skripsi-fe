...
const MultipleSelector = React.forwardRef(
  (
    ...
    }, [arrayDefaultOptions, arrayOptions, groupBy, onSearch, optionsState]);

    useEffect(() => {
      const doSearchSync = () => {
        const res = onSearchSync && onSearchSync(debouncedSearchTerm);
        ...
        <CommandItem
          ...
          onSelect={(value) => {
            if (selected.length >= maxSelected) {
              onMaxSelected && onMaxSelected(selected.length);
              return;
            }
            setInputValue("");
    ...
    const commandFilter = React.useCallback(() => {
      if (commandProps && commandProps.filter) {
      ...
      return undefined;
    }, [creatable, commandProps && commandProps.filter]);
    ...
    return (
      <Command
        ...
        onKeyDown={(e) => {
          handleKeyDown(e);
          commandProps && commandProps.onKeyDown && commandProps.onKeyDown(e);
        }}
        className={cn(
          "h-auto overflow-visible bg-white",
          commandProps && commandProps.className
        )}
        shouldFilter={
          commandProps && commandProps.shouldFilter !== undefined
      ...
      >
          ...
          <div className="relative flex flex-wrap gap-1">
            ...
            <button
              ...
              onClick={() => {
                ...
                onChange && onChange(selected.filter((s) => s.fixed));
              }}
              className={cn(
        ...
        <div className="relative">
          {open && (
            <CommandList
              className="absolute top-1 z-10 bg-white w-full rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in"
